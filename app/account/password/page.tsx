...
const Page = () => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormVal>({mode: "onChange", resolver: zodResolver(validationPassword)});
  ...
  const onSubmit = async (passwordData: FormVal) => {
    ...
    // alert("パスワードを変更しました。再ログインが必要な場合があります。")
    router.push("/");
    return;
  };
  ...
  return (
    <div className="w-full max-w-3xl mx-auto my-16 px-2">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        {errorMsg && <p className="text-red-700 font-semibold mb-4 bg-red-100 border border-red-300 p-2 rounded">{errorMsg}</p>}
        <div className="mb-5">
